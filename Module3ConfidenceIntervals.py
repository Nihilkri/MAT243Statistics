...
def Tstar(c:float, df:int) -> float:
  ...
  criticalValues = [[2.015, 2.571, 4.032],
                    [1.812, 2.228, 3.169],
                    [1.753, 2.131, 2.947],
                    [1.711, 2.064, 2.797],
                    [1.309, 1.694, 2.449]]
...
def MarginOfError(c:float, std:float, n:int, pop:bool = False) -> float:
  ...
  if pop:
    ...
    # Section 3.2 t confidence intervals
    sampleStd = std
    criticalValue = Tstar(c, n - 1)
    standardError = sampleStd / (n ** 0.5)
    return criticalValue * standardError

def ConfidenceInterval(c:float, sampleMean:float, std:float, n:int, pop:bool = False):
  """ Section 3.1: Confidence intervals
...
def section3test():
  section = "Example 3.2.3: Circumference of basketballs"
